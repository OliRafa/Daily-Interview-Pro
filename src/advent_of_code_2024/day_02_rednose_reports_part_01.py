...
INPUTS_FILE_PATH = Path(__file__).parent.joinpath("data", "day_02_inputs.txt")


def transform_inputs(inputs: list[str]) -> Iterable[Iterable[int]]:
    inputs = map(lambda x: x.split(" "), inputs)
    return map(lambda x: map(lambda y: int(y), x), inputs)


def identify_safe_reports(reports: Iterable[Iterable[int]]) -> int:
    num_safe_reports = 0
    for report in reports:
        ...
        try:
            for level in report:
                # It's safe to do this operation because there's no "0" in the inputs
                # list.
                if not previous:
                    previous = level
                    continue

                assert abs(previous - level) >= 1 and abs(previous - level) <= 3
...
result = identify_safe_reports(inputs)
